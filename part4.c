...
int main(int argc, char* argv[]) {
    ...
    if (result < 0) {
        ...
        if ((fd1[0] = open("reading", O_RDONLY)) < 0) {
            printf("Can\'t open for reading\n");
            exit(-1);
        }
        size = read(fd1[0], str, maximum_size);
        if (close(fd1[0]) < 0) {
            printf("parent: Can\'t close reading\n");
            exit(-1);
        }
        ...
        if (close(fd2[1]) < 0) {
            printf("parent: Can\'t close writing\n");
            exit(-1);
        }
    } else {
        // считывание из файла
        ...
        size = read(fd1[1], str, maximum_size);
        if (size == -1) {
            printf("Can\'t read\n");
            exit(-1);
        }
        str[size] = '\0';
        if ((fd1[0] = open("reading", O_WRONLY)) < 0) {
            printf("Can\'t open for writting\n");
            exit(-1);
        }
        size = write(fd1[0], str, size);
        if (close(fd1[0]) < 0) {
            printf("child: Can\'t close writing side\n");
            exit(-1);
        }

        if ((fd2[1] = open("write", O_RDONLY)) < 0) {
            printf("Can\'t open for writting\n");
            exit(-1);
        }
        if ((fd2[0] = open(argv[2], O_WRONLY | O_CREAT)) < 0) {
            printf("Can\'t open for writting\n");
            exit(-1);
        }
        len_ans = read(fd2[1], ans, maximum_size);
        size = write(fd2[0], ans, len_ans);
        if (size != len_ans) {
            printf("Can\'t write all string\n");
            exit(-1);
        }
        if (close(fd2[1]) < 0) {
            printf("child: Can\'t close writing\n");
            exit(-1);
        }
        exit(0);
    }
